...
class inputDevices:
	...
	def getInputDevices(self):
		...
		for evdev in devices:
			try:
				...
				print '[InputDevices] getInputDevices  <ERROR: ioctl(EVIOCGNAME): ' + str(err) + ' >'
				self.name = None
	...
	def setEnabled(self, device, value):
		...
		#print "[InputDevices] setEnabled for device %s to %s from %s" % (device,value,oldval)
	...
	def setName(self, device, value):
		#print "[InputDevices] setName for device %s to %s" % (device,value)
	...
	def setDefaults(self, device):
		print "[InputDevices] setDefaults for device %s" % device
	...
	def setRepeat(self, device, value): #REP_PERIOD
		if self.getDeviceAttribute(device, 'enabled'):
			print "[InputDevices] setRepeat for device %s to %d ms" % (device,value)
	...
	def setDelay(self, device, value): #REP_DELAY
		if self.getDeviceAttribute(device, 'enabled'):
			print "[InputDevices] setDelay for device %s to %d ms" % (device,value)
